...
| **学习卡片** | 显示单词 → 释义页可 **查看例句 / 联想 / 词根分析**（本地生成）→ **认识 / 模糊 / 忘记**。词根分析为**通用引擎**（非逐词硬编码）：拉丁/希腊前缀·词根·后缀贪心匹配 → 现代合成词（sub+set）→ 日耳曼本土词（OE+PIE）；同根词仅按词源血统匹配。覆盖率可用 `node scripts/etymology-coverage.mjs <词书.md>` 查看。 |
